import Header from "../components/Header";
import "../styles/CV.css";

const CV = () => {
    return (
        <>
            <Header isHome={false} />
            <div className="CVContainer">
                SONG MIN JI <br />
                Lives and Works in Seoul <br /> <br />
                songminji1918@gmail.com <br /> <br /> <br />
                학력 <br /> <br />
                2021 서울과학기술대학교 조형예술과 학사 졸업
                <br /> <br /> <br />
                단체전 <br /> <br />
                2023 4 needles, 에브리아트, 서울 <br />
                <br />
                2023 QUEST, 갤러리 인 HQ, 서울
                <br /> <br />
                2022 Peer to Peer, 온수공간, 서울 <br />
                <br />
                2020 미리 찌는 살, WESS, 서울
                <br /> <br />
                2020 인류의 조상은 바나나, 가변크기, 서울 <br /> <br /> <br />
                Education <br /> <br />
                2021 BFA Fine Arts, Seoul National University of Science &
                Technology, Seoul <br /> <br /> <br />
                Group Exhibitions <br /> <br /> 2023 4 needles, Every Art, Seoul{" "}
                <br /> <br />
                2023 QUEST, Gallery IN HQ, Seoul <br /> <br />
                2022 Peer to Peer, ONSU GONG-GAN, Seoul <br /> <br />
                2020 Fatten for Tomorrow, WESS, Seoul <br /> <br /> 2020 인류의
                조상은 바나나, Dimension Variable, Seoul
            </div>
        </>
    );
};

export default CV;
